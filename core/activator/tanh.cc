 ...
 *
 * @brief
 * Tanh implementation : extension of Activator class.
 * Computes Tanh function given input.
 */
...
Matrix Tanh::activate(const Matrix &input) const {
    return input.exponent().subtract(input.scalarMultiply(-1.0).exponent())
                .multiplyElementwise(input.exponent().add(input.scalarMultiply(-1.0).exponent()).toPower(-1));
}
